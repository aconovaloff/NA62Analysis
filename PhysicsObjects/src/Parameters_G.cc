...
Parameters_G* Parameters_G::fInstance = 0;
...
{ 
  ...
  // SAV parameters
  fSAVChannelT0 = new Double_t[14];
  fIsSAVStored = 0;
}

int Parameters_G::LoadParameters_G(TString fileparname) {
  ...
  while(Line.ReadLine(parFile)) {
    ...
    fParameters_G.insert(std::pair<TString,double>(name,value));    
  }
  parFile.close();
  return 1;
}
...
Parameters_G *Parameters_G::GetInstance() {
  if (fInstance==0) {
    fInstance = new Parameters_G();
  }

  return fInstance;
}

double Parameters_G::GetValue(TString name) {
  NA62Analysis::NA62Map<TString,double>::type::const_iterator ptr;
  if ((ptr=fParameters_G.find(name))!=fParameters_G.end()) return ptr->second;
  return -9999999.;
}

// Detector related parameters repository //

void Parameters_G::StoreCHODParameters_G() {
  ...
  for (Int_t jpair=0; jpair<2048; jpair++) {
    ...
    TString namevar4;  namevar4.Form("finet0%d",jpair);
    ...
    Double_t finet0 = GetValue(namevar4.Data());
    if (fabs(t0)>999) t0 = 0.0;
    fCHODAllT0[islab][ip] = t0;
    fCHODAllFineT0[islab][ip] = finet0;
...
void Parameters_G::StoreRICHParameters_G() {
...
void Parameters_G::StoreLKrParameters_G() {
...
void Parameters_G::StoreGigaTrackerParameters_G() {
...
void Parameters_G::StoreLAVParameters_G() {
...
void Parameters_G::StoreSACParameters_G() {
...
void Parameters_G::StoreSAVParameters_G() {
  if (fIsSAVStored) return;
  for (Int_t jch=0; jch<14; jch++) {
    TString namevar; namevar.Form("sav_channelt0_%d",jch);
    Double_t t0 = GetValue(namevar.Data());
    if (t0==-9999999.) t0 = 0;
    fSAVChannelT0[jch] = t0; 
  }
  fIsSAVStored = 1;
}
